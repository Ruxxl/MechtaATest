...
describe('Редактирование профиля — синхронизация после сохранения (TC-080..083)', () => {
    ...
    // TC-080 — БАГ (см. BUG-025): карточка ФИО в сайдбаре ДОЛЖНА обновиться сразу
    // после сохранения, без перезагрузки страницы. В реальности — не обновляется
    // (сама модалка при этом корректно перечитывает новые данные). Тест ожидает
    // корректное поведение и падает, пока баг не исправлен.
    it('Карточка профиля обновляется сразу после сохранения, без перезагрузки (BUG-025)', () => {
        EditProfile.visitAndOpen();
        EditProfile.setFirstname('Пётр');
        EditProfile.setLastname('Сидоров');
        EditProfile.save();
        ...
        // Баг: без reload карточка ФИО (первый h2 страницы) остаётся старой
        cy.get('h2').first().should('have.text', 'Пётр Сидоров');
    });

    // TC-081 — данные персистентны на бэкенде (подтверждается повторным GET после
    // reload) — эта часть РАБОТАЕТ корректно, отдельно от BUG-025 (которая только
    // про отсутствие живого обновления без reload)
    ...
    // данных. Проверяем через API + reload (не через живой h2 без reload — это
    // отдельно сломано в BUG-025 и уже покрыто тестом выше).
...
describe('БАГ-024: тихий silent-drop при PUT /v3/profile для цифр/HTML в имени', () => {
    beforeEach(() => {
        cy.loginD2();
    });
    ...
    // TC-008/TC-012/TC-030 — сервер должен вести себя так же, как для остальных
    // невалидных случаев на этом же эндпоинте (пустое имя, будущая дата, битый
    // email, неверный gender — все корректно дают 422). Тест ожидает 422 и падает,
    // пока баг не исправлен: реально сервер отвечает 204, будто сохранил.
    it('PUT /v3/profile с цифрами в firstname должен отклоняться 422, как остальные невалидные значения (BUG-024)', () => {
        cabinetApi.putProfile({ ...BASELINE, firstname: 'Иван123' }, { failOnStatusCode: false }).then((res) => {
            expect(res.status, 'ожидали 422, как для пустого имени/будущей даты/битого email').to.eq(422);
        });
    });

    // Тот же баг для Фамилии — не единичный случай конкретного поля
    it('PUT /v3/profile с XSS-пейлоадом в lastname должен отклоняться 422 (BUG-024)', () => {
        cabinetApi.putProfile({ ...BASELINE, lastname: '<script>alert(1)</script>' }, { failOnStatusCode: false }).then((res) => {
            expect(res.status).to.eq(422);
        });
    });

    // Тот же баг, но через реальный UI. ВАЖНО про стиль этого теста (отличается
    // от остальных): тост "Успешно" на этой странице сам скрывается через
    // несколько секунд (auto-dismiss) — поэтому негативная проверка
    // `.should('not.exist')` в принципе ненадёжна: она может пройти либо
    // слишком рано (до монтирования тоста), либо слишком поздно (уже после
    // авто-скрытия), в обоих случаях давая ложный "зелёный" результат вне
    // зависимости от бага. Поэтому здесь, в порядке исключения, тест fixирует
    // ФАКТИЧЕСКОЕ (баговое) поведение через надёжную позитивную проверку
    // `.should('be.visible')` (у неё есть настоящий retry-ability) — тест
    // ПРОХОДИТ, пока баг воспроизводится, и красный статус самого бага
    // подтверждают два теста выше (PUT возвращает 204 вместо 422).
    it('UI показывает тост «Успешно» при сохранении цифр в Имени, хотя значение не применилось (BUG-024, факт)', () => {
        EditProfile.visitAndOpen();
...
describe('Редактирование профиля — серверная валидация напрямую через API', () => {
    ...
    it('Будущая дата рождения отклоняется с 422 и не сохраняется', () => {
        cabinetApi.putProfile({ ...BASELINE, birthdate: '2026-08-08' }, { failOnStatusCode: false }).then((res) => {
            expect(res.status).to.eq(422);
