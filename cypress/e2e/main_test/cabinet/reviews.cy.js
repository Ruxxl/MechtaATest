...
// «Мои отзывы» найден BUG-035 (High, Jira AS-4561): карточка отзыва не
// показывает название товара, теги и (на десктопном viewport) сам текст
// отзыва, плюс превью-картинка товара всегда битая — см.
// BugReport/Личный кабинет/reviews/BUG-035-my-reviews-card-missing-fields.md.
...
// Часть полей (статус, рейтинг, дата, лайки/дизлайки, название товара,
// превью товара) отображается корректно — см. тесты ниже. Часть полей —
// текст отзыва И собственные фото отзыва на десктопе, а также теги — НЕ
// отображаются, задокументировано как BUG-035 (Jira AS-4561), см. второй
// describe-блок ниже.
//
...
// этой подгрузки — см. секцию «Уточнение» в BUG-035.md.
describe('Отзывы — вкладка «Мои отзывы»: соответствие карточки данным API', () => {
    ...
    // сами дожидаются появления, поэтому явного sleep не требуется. См.
    // «Уточнение» в BUG-035.md — изначально эти поля ошибочно считались
    // частью бага.
...
// BUG-035 (High, Jira AS-4561): карточка отзыва на «Мои отзывы» не
// показывает текст отзыва, теги и собственные фото отзыва на десктопном
// viewport (текст и фото — внутри одного и того же mobile-only
// `div.block.lg:hidden` без десктопного аналога; теги вообще не в
// разметке — сверено разведкой 2026-08-11). Тесты ниже фиксируют
// ОЖИДАЕМОЕ поведение — сейчас падают, задокументированно.
describe('Отзывы — вкладка «Мои отзывы»: BUG-035 — отсутствующие поля карточки', () => {
    beforeEach(() => {
        cy.loginD2();
        Reviews.visit();
        Reviews.getMyReviewsTab().click();
    ...
    it('BUG-035: текст отзыва (body) должен быть виден на десктопном viewport', () => {
        cabinetApi.getPersonalReviewsV3().then(({ body }) => {
            body.reviews.forEach((review) => {
                cy.contains(review.body).should('be.visible');
            });
        });
    });

    it('BUG-035: теги отзыва (tags[]) должны быть видны на карточке', () => {
        cabinetApi.getPersonalReviewsV3().then(({ body }) => {
            const reviewWithTags = body.reviews.find((r) => (r.tags || []).length > 0);
            expect(reviewWithTags, 'baseline: среди реальных отзывов есть хотя бы один с тегами').to.exist;
            reviewWithTags.tags.forEach((tag) => {
                cy.contains(tag.name).should('be.visible');
            });
        });
    });

    // Разведка 2026-08-11 показала: собственные фото, приложенные
    // пользователем к отзыву (photos[].thumb), физически присутствуют в
    // DOM с верным src, но лежат ВНУТРИ того же `div.block.lg:hidden`, что
    // и текст отзыва — то есть тоже невидимы на десктопе. Изначально этот
    // тест был написан как "работает корректно", но при живом прогоне сам
    // выявил, что это не так — перенесён сюда как часть BUG-035.
    it('BUG-035: собственные фото отзыва (photos[]) должны быть видны на десктопном viewport', () => {
        cabinetApi.getPersonalReviewsV3().then(({ body }) => {
            const totalPhotos = body.reviews.reduce((sum, r) => sum + (r.photos || []).length, 0);
            expect(totalPhotos, 'baseline: среди реальных отзывов есть хотя бы одно фото').to.be.greaterThan(0);

            const allThumbs = body.reviews.flatMap((r) => (r.photos || []).map((p) => p.thumb));
            allThumbs.forEach((thumbUrl) => {
                cy.get(`img[src="${thumbUrl}"]`).should('be.visible');
            });
        });
    });
});
