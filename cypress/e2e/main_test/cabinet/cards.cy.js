...
// кнопка "Добавить карту" на этом стенде не открывает рабочую форму
// (платёжный провайдер не настроен в тестовом окружении — подтверждено
// пользователем 2026-08-10 как ограничение стенда, не баг, см. BUG-016 в
// ../BugReport, отклонён) — то есть реальное удаление этой карты
// необратимо для будущих тестов (новую карту добавить будет нечем).
// Поэтому здесь НЕ выполняется фактическое подтверждение удаления (только
// открытие/отмена диалога) и НЕ тестируется добавление карты через
// реальный ввод платёжных данных (это и запрещено правилами безопасности —
// номера карт/CVC никогда не вводятся, даже тестовые). TC-МО-9..11,
// TC-МО-22..25 помечены как заблокированные ниже.
...
describe('Мои карты — общие элементы страницы', () => {
    ...
    // BUG-015 ИСПРАВЛЕН (проверено 2026-08-10): h1 теперь реально виден на
    // десктопе (display:block, не просто присутствует где-то в скрытом виде).
    it('Заголовок «Мои карты» виден на десктопном вьюпорте', () => {
...
describe('Мои карты — добавление карты', () => {
    ...
    // TC-МО-21 — БАГ-кандидат BUG-016, отклонён 2026-08-10: модалка
    // открывается, но внутри неё — пустой iframe (zoid-загрузчик виджета
    // провайдера с незаполненным src). Подтверждено пользователем — это
    // ограничение конфигурации именно тестового стенда d2.im.mdev.kz
    // (платёжный провайдер не настроен в тестовом окружении), на боевом
    // окружении не воспроизводится. Тест удалён — не имеет смысла
    // перманентно падать по стенд-специфичной, не багу причине.
});
